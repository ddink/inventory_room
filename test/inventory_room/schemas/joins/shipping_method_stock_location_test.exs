defmodule InventoryRoom.Joins.ShippingMethodStockLocationTest do
  use InventoryRoom.DataCase, async: true
  alias Ecto.Changeset
  import InventoryRoom.Joins.ShippingMethodStockLocation

  describe "changeset/1" do
    test "success: returns a valid changeset when given valid arguments" do
      params = Factory.string_params_for(:shipping_method_stock_location)

      assert_valid_changeset(params, shipping_method_stock_location_fields(), &changeset/1)
    end

    test "error: returns an error changeset when given un-castable values" do
      params = bad_shipping_method_stock_location_params()

      assert_invalid_changeset(
        params,
        shipping_method_stock_location_fields(),
        &changeset/1,
        :cast
      )
    end
