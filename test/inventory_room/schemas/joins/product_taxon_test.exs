defmodule InventoryRoom.Joins.ProductTaxonTest do
  use InventoryRoom.DataCase, async: true
  alias Ecto.Changeset
  import InventoryRoom.Joins.ProductTaxon

  describe "changeset/1" do
    test "success: returns a valid changeset when given valid arguments" do
      params = Factory.string_params_for(:product_taxon)

      assert_valid_changeset(params, product_taxon_fields(), &changeset/1)
    end

    test "error: returns an error changeset when given un-castable values" do
      params = bad_product_taxon_params()

      assert_invalid_changeset(
        params,
        product_taxon_fields(),
        &changeset/1,
        :cast
      )
    end
