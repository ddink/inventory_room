defmodule InventoryRoom.Joins.StorePaymentMethodTest do
  use InventoryRoom.DataCase, async: true
  alias Ecto.Changeset
  import InventoryRoom.Joins.StorePaymentMethod

  describe "changeset/1" do
    test "success: returns a valid changeset when given valid arguments" do
      params = Factory.string_params_for(:store_payment_method)

      assert_valid_changeset(params, store_payment_method_fields(), &changeset/1)
    end

    test "error: returns an error changeset when given un-castable values" do
      params = bad_store_payment_method_params()

      assert_invalid_changeset(
        params,
        store_payment_method_fields(),
        &changeset/1,
        :cast
      )
    end
