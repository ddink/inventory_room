defmodule InventoryRoom.Joins.ProductOptionTypeTest do
  use InventoryRoom.DataCase, async: true
  alias Ecto.Changeset
  import InventoryRoom.Joins.ProductOptionType

  describe "changeset/1" do
    test "success: returns a valid changeset when given valid arguments" do
      params = Factory.string_params_for(:product_option_type)

      assert_valid_changeset(params, product_option_type_fields(), &changeset/1)
    end

    test "error: returns an error changeset when given un-castable values" do
      params = bad_product_option_type_params()

      assert_invalid_changeset(
        params,
        product_option_type_fields(),
        &changeset/1,
        :cast
      )
    end
