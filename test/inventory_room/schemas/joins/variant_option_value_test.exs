defmodule InventoryRoom.Joins.VariantOptionValueTest do
  use InventoryRoom.DataCase, async: true
  alias Ecto.Changeset
  import InventoryRoom.Joins.VariantOptionValue

  describe "changeset/1" do
    test "success: returns a valid changeset when given valid arguments" do
      params = Factory.string_params_for(:variant_option_value)

      assert_valid_changeset(params, variant_option_value_fields(), &changeset/1)
    end

    test "error: returns an error changeset when given un-castable values" do
      params = bad_variant_option_value_params()
