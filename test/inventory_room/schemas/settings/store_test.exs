defmodule InventoryRoom.Settings.StoreTest do
  use InventoryRoom.DataCase, async: true
  import InventoryRoom.Settings.Store
  alias Ecto.Changeset

  describe "changeset/1" do
    test "success: returns a valid changeset when given valid arguments" do
      params = Factory.string_params_for(:store)

      assert_valid_changeset(params, store_fields(), &changeset/1)
    end

    test "error: returns an error changeset when given un-castable values" do
      params = bad_store_params()
  ...
  describe "create_changeset/1" do
    test "success: returns a valid changeset when given valid arguments" do
      ...
      assert_valid_changeset(params, store_fields(), &create_changeset/1)
    end
